...
if not st.session_state["authenticated"]:
    ...
    st.markdown("### Alat Konversi LaTeX - Docx & PDF")
...
st.title("📄 Konverter LaTeX ke Word & PDF")
...
st.markdown("""
    ...
    /* Style tambahan untuk tombol download agar sejajar rapi */
...
if uploaded_file is not None:
    ...
    st.write("⏳ Sedang memproses konversi...")
    
    try:
        output_docx = "hasil_konversi.docx"
        output_pdf = "hasil_konversi.pdf"

        # 1. Konversi ke DOCX (Word)
        pypandoc.convert_file("input.tex", 'docx', outputfile=output_docx, extra_args=['--mathml'])
        
        # 2. Konversi ke PDF
        # NOTE: Ini memerlukan LaTeX engine (seperti MikTeX atau TeXLive) terinstall di sistem.
        pypandoc.convert_file("input.tex", 'pdf', outputfile=output_pdf, extra_args=['--pdf-engine=pdflatex'])
        
        st.success("✅ BERHASIL! Silakan pilih format download:")
        
        # Membuat 2 Kolom agar tombol download berdampingan
        col1, col2 = st.columns(2)

        # Kolom 1: Download Word
        with col1:
        ...
        # Kolom 2: Download PDF
        with col2:
            with open(output_pdf, "rb") as file:
                st.download_button(
                    label="📥 DOWNLOAD PDF (.PDF)",
                    data=file,
                    file_name=uploaded_file.name.replace('.tex', '.pdf'),
        ...
        st.error("❌ Gagal Konversi PDF: Sistem tidak menemukan instalasi LaTeX (pdflatex). Pastikan TeXLive atau MiKTeX sudah terinstall.")
        # Tetap tampilkan tombol Word jika PDF gagal tapi Word berhasil (opsional, butuh logic tambahan)
