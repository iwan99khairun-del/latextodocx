...
# --- SETUP HALAMAN ---
st.set_page_config(page_title="Replika R + Resolusi", layout="centered")
st.title("📊 Replika R (Fitur Resolusi)")
...
if uploaded_file:
    try:
        ...
        if x_col and y_col:
            ...
            with st.expander("⚙️ Buka Pengaturan Gambar", expanded=True):
                col_s1, col_s2 = st.columns(2)
                with col_s1:
                    fig_w = st.slider("Lebar (Width)", 3.0, 10.0, 5.0)
                    fig_h = st.slider("Tinggi (Height)", 3.0, 10.0, 4.0)
                with col_s2:
                    seed = st.number_input("Kode Posisi Titik (Seed)", value=42)
                    jitter = st.slider("Sebaran Titik (Jitter)", 0.0, 0.3, 0.12)
